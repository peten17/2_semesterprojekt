...
import com.ur.urcap.api.domain.value.*;
...
public class EllipseProgramNodeContribution implements ProgramNodeContribution {
	...
	public InputButton buttonSelectPose;
	...
	public EllipseProgramNodeContribution(URCapAPI urCapAPI, DataModel dataModel) {
		...
		this.is_defined = dataModel.get(DEFINED_KEY, false);
	}
	...
	private void resetWaypointNodes() {
		...
		for (WaypointNode waypointNode : waypointNodes) {
			WaypointNodeConfig waypointNodeConfig = waypointNode.getConfigFactory().createFixedPositionConfig(blendParameters, motionParameters);
	...
	private void configureWaypointNodes(Pose centerPose, JointPositions jointPositions) {
		...
		for (WaypointNode waypointNode : waypointNodes) {
			...
			WaypointNodeConfig newWaypointNodeConfig = createWaypointConfig(centerPose, jointPositions, offsetX, offsetY, offsetZ);
			waypointNode.setConfig(newWaypointNodeConfig);
		}
	}

	private WaypointNodeConfig createWaypointConfig(Pose centerPose, JointPositions jointPositions, double xOffsetInMM, double yOffsetInMM, double zOffsetInMM) {
		BlendParameters blendParameters = waypointNodeConfigFactory.createSharedBlendParameters();
		WaypointMotionParameters motionParameters = waypointNodeConfigFactory.createSharedMotionParameters();
	...
	private Pose createPoseUsingCenterPoseAndOffset(Pose pose, double xOffset, double yOffset, double zOffset, Length.Unit unit) {
	...
	private void createNodes() {
		...
		try {
			moveNode = programNodeFactory.createMoveNode();
			TreeNode rootTreeNode = programModel.getRootTreeNode(this);
			moveTreeNode = rootTreeNode.addChild(moveNode);

			WaypointNode waypointNode = (WaypointNode) moveTreeNode.getChildren().get(0).getProgramNode();
	...
	private void setDefined(boolean defined) {
		...
		dataModel.set(DEFINED_KEY, is_defined);
	}
}
